...
- [前端整理合集](#前端整理合集)
  ...
  - [js](#js)
    ...
    - [浏览器消息通知](#浏览器消息通知)
    - [说说对js引擎的理解](#说说对js引擎的理解)
      - [比较常见的JavaScript引擎有哪些呢？](#比较常见的javascript引擎有哪些呢)
      - [浏览器内核与JS引擎关系](#浏览器内核与js引擎关系)
    - [V8引擎的优化](#v8引擎的优化)
    - [V8的转化代码过程](#v8的转化代码过程)
    - [AST前端的应用场景](#ast前端的应用场景)
  - [ES6](#es6)
  ...
  - [node](#node)
    ...
    - [koa洋葱模型](#koa洋葱模型)
  - [包管理工具](#包管理工具)
    - [多个项目不同的node版本处理](#多个项目不同的node版本处理)
...
### css选择器
id选择器 标签选择器 类选择器 后代选择器 子选择器(div>p) 相邻选择器(a+div) 群组选择器(div,p 选择所有div和p) 伪类选择器 伪元素选择器 属性选择器 层级选择器(p~ul 选择前面有p元素的所有ul) 通配符
...
### postcss
PostCSS 是一个使用 JavaScript 工具和插件转换 CSS 代码的工具。它允许你使用未来的 CSS 语法，并为你提供许多有用的功能，如变量、嵌套规则、混合（mixins）、函数等，这些功能在原生 CSS 中是不可用的，可以添加前缀和低浏览器适配。
...
1. 从属关系：
	+ link：是HTML提供的标签，`不仅可以加载CSS文件，还可以定义RSS、rel连接属性等`。
		- `<link rel="rss" type="application/rss+xml" title="My Website RSS Feed" href="http://example.com/rss.xml">`
	+ @import：是CSS提供的语法规则，主要用于导入样式表。
2. 加载顺序：
	+ link：在页面加载时，link标签引入的CSS会被同时加载。
	+ @import：**@import引入的CSS将在页面加载完毕后被加载**。这意味着当使用@import时，用户可能会在页面加载的初期看到`一个没有样式的页面`。
3. ~~兼容性：~~
	+ link：作为HTML元素，不存在兼容性问题。
	+ ~~@import：是CSS2.1才有的语法，因此只能在IE5+等现代浏览器中识别。在一些旧版或非主流浏览器中可能存在兼容性问题。~~
4. DOM可控性：
	+ link：因为是HTML元素，`可以通过JavaScript DOM动态的添加样式。`
	+ @import：则不支持通过JavaScript DOM进行动态操作。
5. 样式表引用：
	+ @import：可以在CSS中再次引入其他样式表，如在一个主样式表中引入多个子样式表。然而，`这样做可能会增加服务器的HTTP请求数量，从而增加服务器压力。`
	+ link：则通常直接链接到CSS文件，不支持在CSS中再次引入其他样式表。
6. 书写方式：
...
escape、encodeURI、encodeURIComponent这三个函数在JavaScript中用于处理字符串的编码

1. 函数用途：
	+ ~~escape：这个函数主要用于对非ASCII字符进行编码，但在现代Web开发中已被弃用，因为它对非ASCII字符的编码方式与其他函数不一致，且存在安全风险。~~
	+ encodeURI：这个函数用于对完整的URI进行编码，它不会对URI中的特殊字符（如:/? #&=+等）进行编码，从而确保URI的结构不会被破坏。
	+ encodeURIComponent：这个函数用于对URI的组件（如查询参数）进行编码，它会对URI中的特殊字符进行编码，从而确保这些组件在传输过程中不会被误解或导致安全漏洞。
2. 编码字符范围：
	+ escape：对ASCII字母数字字符不进行编码，对特殊字符进行编码，对非ASCII字符进行编码（但方式与其他函数不一致）。
	+ encodeURI：不会对ASCII字母数字字符、~!@#$&*()=:/,;?+ 这些特殊字符进行编码。
	+ encodeURIComponent：不会对ASCII字母数字字符、~!*()' 这些特殊字符进行编码。
3. 使用场景：
	+ ~~由于escape已被弃用，因此在实际开发中应尽量避免使用。~~
	+ 当需要编码整个URI时，应使用encodeURI。
	+ 当需要编码URI的组件（`如查询参数`）时，应使用encodeURIComponent。
4. 优缺点：
	+ escape：优点是过去在代码中广泛使用，但缺点是已被弃用，且对非ASCII字符的编码方式不一致。
	+ encodeURI：优点是适用于编码整个URI，缺点是不能用于编码URI的组件。
...
![1](book_files/329.jpg)

#### 比较常见的JavaScript引擎有哪些呢？
+ SpiderMonkey：第一款JavaScript引擎（JavaScript作者）;
+ Chakra：微软开发，用于IT浏览器；
...
V8引擎实现了`即时编译（JIT）`，将JS代码直接编译成机器码，而非先转换为字节码。少了一层转换消耗，明显的提升运行效率。（某些场合也会转字节码，属于非必要可选操作）

V8持续在性能优化、内存管理和现代JavaScript特性支持方面进行创新。

![1](book_files/331.jpg)


+ 解析 (Parse): 输入的JS源代码首先被解析器(Parse)处理，解析成抽象语法树（AST）。这一步是编程语言的常规处理过程，目的是将源代码转换成结构化的、便于进一步处理的内部表示形式
+ 生成字节码 (Bytecode): AST 接下来被 Ignition（V8的解释器）处理，转换成字节码。字节码是介于源代码和机器代码之间的一种低级语言，比源代码更接近机器语言，但不是特定于任何单一的硬件
+ 执行字节码 (Execution): 字节码随后被执行。在这一阶段，代码运行并进行实际的计算
+ 优化 (Optimization): TurboFan（V8的优化编译器）根据运行时的数据对字节码进行优化，生成优化的机器代码。优化的目的是提高代码的执行效率，通过诸如内联函数、消除冗余代码等技术来增强性能(比如一个函数被多次调用，那么就会被标记为`热点函数`，那么就会经过TurboFan转换成优化的机器码，提高代码的执行性能；)
+ 去优化 (Deoptimization): 如果优化的假设在实际运行中不成立，优化的代码需要被去优化，回退到字节码执行，这一过程称为去优化(比如sum函数原来执行的是number类型，后来执行变成了string类型)[去优化步骤虽然优化后的机器码的使用被暂停，代码执行回退到解释器中执行未优化的字节码，但如果后续的性能分析认为有必要，这些代码还是会被重新编译成机器代码，因为最后的执行位置是CPU，而CPU只接受机器代码；]
+ 生成机器代码 (Machine Code): 优化后的代码被转换为机器代码，即直接由计算机硬件执行的代码。这一步骤确保代码运行得尽可能快
...
![2](book_files/332.jpg)

在这个JS引擎的处理流程中，并不是一蹴而就的将JS代码直接转化为机器语言，在这种中间还有一层字节码

字节码为运行时提供了一个更加安全和控制的环境，可以有效地隔离恶意代码和系统资源。V8引擎可以更容易实施安全策略和内存管理，毕竟我们不知道用户会传递进来什么样子的代码

还有就是程序员核心能力之一的调试，`通过字节码可以更容易地实现代码的逐步执行和调试断点，如果在机器代码中进行的话会很复杂`。经过综合的权衡，V8引擎最终在这方面做出了取舍

首先在生成字节码的时候，我们说不特定于任何单一硬件。这体现了其中的一个优势：跨平台兼容，因为无需针对每种硬件配置重新编译代码，就可以适应不同设备和操作系统

并且如下图在处理流程第四步中有进行优化，减轻了多一层转化流程所造成的性能影响，第五步则是对优化好的代码进行检测，看是否成立，不成立就会回退反复进行，直到成立后才生成机器代码

![3](book_files/333.jpg)


### V8的转化代码过程

![2](book_files/335.jpg)

![3](book_files/336.jpg)


### AST前端的应用场景
AST（Abstract Syntax Tree，抽象语法树）在前端开发中的应用场景非常广泛。以下是一些主要的应用场景：

1. 代码转译和优化

代码转译：在前端开发中，经常需要将一种语法的代码转换为另一种语法的代码，以满足不同浏览器或环境的兼容性需求。例如，Babel是一个流行的JavaScript编译器，它使用AST将现代JavaScript代码（如ES6+）转换为兼容性更好的旧版本JavaScript代码。

代码优化：通过解析源代码生成AST，可以对代码进行优化，如去除未使用的变量、合并相似的语句、压缩变量名等，以减少代码体积并提高执行效率。工具如UglifyJS和Terser就是基于AST进行代码优化的例子。

2. 代码分析和检查

静态代码分析：ESLint是一个静态代码分析工具，它通过生成AST来检查JavaScript代码是否符合编码规范和最佳实践。这有助于开发者在代码运行之前发现并修复潜在的问题。

代码风格检查：Prettier等工具通过解析代码生成AST，然后重新生成格式化后的代码，以确保代码风格的一致性和可读性。

3. 开发框架和库的支持

框架编译：在React等前端框架中，JSX是一种JavaScript的语法扩展，用于描述UI结构。Babel等工具将JSX解析为AST，然后转换为标准的JavaScript代码，以便浏览器能够理解和执行。

库和工具的构建：许多前端库和工具（如Webpack、Rollup等）都使用AST来分析和转换代码，以实现模块打包、代码分割、懒加载等功能。

4. 代码迁移和重构

代码迁移：在将旧代码迁移到新版本的前端框架或库时，AST工具（如Codemods）可以帮助开发者自动修改代码，以减少手动迁移的工作量并提高迁移的准确性。

代码重构：通过操作AST，开发者可以对代码进行大规模的重构，如改变函数签名、调整类结构等，而无需担心手动修改可能引入的错误。

5. 编辑器插件和IDE支持

语法高亮：许多代码编辑器和IDE都使用AST来提供语法高亮功能，使代码更加易于阅读和理解。

代码补全：基于AST的代码补全功能可以更准确地预测开发者想要编写的代码，并提供相应的补全建议。

6. 性能优化

代码分割：通过AST分析，可以将代码分割成多个块（chunks），以便在需要时动态加载，从而提高应用的加载速度和性能。

代码压缩：如上所述，AST优化工具可以通过去除未使用的代码、压缩变量名等方式来减小代码体积，进而提升应用的加载和执行性能。

综上所述，AST在前端开发中的应用场景涵盖了代码转译、优化、分析、检查、框架和库支持、代码迁移和重构、编辑器插件和IDE支持以及性能优化等多个方面。理解和掌握AST的原理和应用，对于前端开发者来说是非常重要的。

实际应用：eslint babel jscodeshift (代码迁移重构)

![jscodeshift](book_files/334.jpg)
...
```bash
#  处理单个文件
jscodeshift -t remove-console.js path/to/your/file.js
#  处理目录中的所有文件
jscodeshift -t remove-console.js path/to/your/directory --extensions js
...
2. http1.1 是 http1.0 的改进版，它做出了以下改进：
...
### 简单讲解一下 http2 的多路复用 
在 HTTP/2 中，有两个非常重要的概念，分别是帧（frame）和流（stream）。 帧代表着最小的数据单位，每个帧会标识出该帧属于哪个流，流也就是多个帧组成的数据流。 多路复用，就是在一个 TCP 连接中可以存在多条流。
...
### 谈谈对svg的理解
SVG（Scalable Vector Graphics）是一种基于XML语法的图像格式，用于描述二维矢量图形和矢量/混合图像。

矢量图形：与基于像素的位图图像（如JPEG或PNG）不同，SVG图像是由数学公式定义的形状、线条、文本和颜色组成。因此，SVG图像可以无限缩放而不损失清晰度，因为它们是基于形状的，而不是基于固定大小的像素网格。

可编辑性：由于SVG是基于文本的XML格式，因此可以使用任何文本编辑器进行编辑。这意味着可以轻松地修改SVG图像的颜色、形状、大小等属性，而无需使用专门的图像编辑软件。

可访问性：SVG图像中的文本可以被搜索引擎索引，并且可以通过屏幕阅读器等辅助技术为视觉障碍用户提供信息。此外，SVG还支持各种交互性，如动画、事件处理等。
...
微前端作为用户界面的一部分，通常由许多组件组成，并使用类似于React、Vue和Angular等框架来渲染组件。每个微前端可以由`不同的团队`进行管理，并可以自主选择框架。虽然在迁移或测试时可以添加额外的框架，出于实用性考虑，建议只使用一种框架。

每个微前端都拥有独立的git仓库、package.json和构建工具配置。因此，每个微前端都拥有独立的构建进程和独立的部署/CI。这通常意味着，每个仓库能快速构建。

常见的微前端框架：qiankun|single-spa|Piral|wujie|@micro-zoe/micro-app
...
![1](book_files/306.jpg)

![2](book_files/307.jpg)

![3](book_files/308.jpg)


### qiankun

![1](book_files/309.jpg)
